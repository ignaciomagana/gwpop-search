...
def test_jax_shape_matches_numpy_reference():
    ...
    fn = build_shape_log_likelihood(pe, sel, density_jax)
...
def test_jax_event_and_selection_terms_match_numpy_components():
    ...
    np.testing.assert_allclose(
        np.asarray(event_terms),
        ref.terms.events.log_likelihoods,
        atol=2e-12,
    )
    np.testing.assert_allclose(
        float(log_exp),
        ref.terms.selection.log_exposure,
        atol=2e-12,
    )
...
def test_jax_selection_chunking_is_invariant():
    ...
    for size in (None, 1, 4, 7, 100):
        ...
        fn = build_shape_log_likelihood(pe, sel, density_jax, config=cfg)
...
def test_jax_likelihood_is_differentiable():
    pe = make_toy_posterior_catalog()
    sel = make_toy_selection_catalog()
    fn = build_shape_log_likelihood(
        pe,
        sel,
        density_jax,
        config=HBIConfig(selection_chunk_size=5),
    )
